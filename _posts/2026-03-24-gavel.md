...
This writeup documents the exploitation of the Gavel machine, an auction management system vulnerable to SQL injection and arbitrary code execution through insecure rule evaluation. The vulnerability chain begins with source code disclosure via `.git` directory exposure, leads to SQL injection for credential extraction, and culminates in remote code execution through unsafe PHP function evaluation. The privilege escalation includes submitting a malicious YAML file to overwrite php configuration file to allow command exectuion.
...
- `-b`: Exclude these HTTP status codes from output (reducing noise from non-existent pages)
- `-t`: Number of concurrent threads (40 threads for faster enumeration)
...
**Result:** We have achieved remote code execution as the `www-data` user (web server user context).
...
**Important Note:** The shell prompt may disappear when switching users in this context. This is normal behavior.
...
```
$ cd /tmp
$ mkdir pwn_exploit && cd pwn_exploit
...
The **Gavel** machine demonstrates a complete attack chain exploiting multiple critical vulnerabilities:

1. **Source Code Disclosure** (`.git` exposure) - Allowed complete application analysis
2. **SQL Injection** - Enabled credential extraction from the database
3. **Weak Password Storage** - Bcrypt hash cracked to obtain admin credentials
4. **Unsafe Code Evaluation** - PHP's `runkit` functions exploited for RCE
5. **Insufficient PHP Hardening** - Restrictive PHP configuration bypassed through YAML processing
6. **Privilege Escalation via SUID** - Root-owned daemon executed attacker-controlled code

**Key Takeaways:**
- Never expose `.git` directories publicly
- Always use parameterized queries to prevent SQL injection
- Never dynamically evaluate user input as code
- Implement proper input validation and sanitization
- Restrict dangerous PHP functions appropriately
- Monitor group-based file permissions and setuid binaries
